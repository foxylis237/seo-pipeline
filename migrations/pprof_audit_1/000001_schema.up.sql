-- =============================================================================
-- Схема pprof_audit_1 целиком, одним файлом.
--
-- ВАЖНО: применяется ТОЛЬКО к схеме pprof_audit_1.
--
--   docker exec -i seo-postgres psql -U seo -d seo \
--     -v ON_ERROR_STOP=1 -c 'SET search_path TO pprof_audit_1' -f - \
--     < migrations/pprof_audit_1/000001_schema.up.sql
--
-- Почему таблица одна, а не набор из article_inputs, article_outputs и article_metadata,
-- как у задач генерации: pprof_audit_1 не создаёт страницу и не правит её, а проверяет уже
