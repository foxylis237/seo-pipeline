-- =============================================================================
-- Схема pprof_fix_7 целиком, одним файлом.
--
-- ВАЖНО: применяется ТОЛЬКО к схеме pprof_fix_7.
--
--   docker exec -i seo-postgres psql -U seo -d seo \
--     -v ON_ERROR_STOP=1 -c 'SET search_path TO pprof_fix_7' -f - \
--     < migrations/pprof_fix_7/000001_schema.up.sql
--
-- Почему таблица одна, а не набор из article_inputs, article_outputs и article_metadata,
-- как у остальных задач: pprof_fix_7 не создаёт статью, а правит уже опубликованную. У неё
...
CREATE TABLE IF NOT EXISTS articles (
    ...
    -- форме, — но у pprof_fix_7 значения совпадают: название записи она не меняет (меняется
    -- заголовок последнего раздела внутри текста), и new_title равен old_title. Так видно,
    -- что заголовок прочитан, а не потерян.
    old_title TEXT,
    new_title TEXT,
