-- =============================================================================
-- Схема obuch_1 целиком, одним файлом.
--
-- ВАЖНО: применяется ТОЛЬКО к схеме obuch_1 и заменяет собой общий каталог migrations/.
--
--   docker exec -i seo-postgres psql -U seo -d seo \
--     -v ON_ERROR_STOP=1 -c 'SET search_path TO obuch_1' -f - \
--     < migrations/obuch_1/000001_schema.up.sql
--
-- Схема описана одним файлом, а не собрана из истории: у obuch_1 нет прошлого, к которому
-- эту схему приводили бы шаг за шагом. Каждая команда идемпотентна (IF NOT EXISTS), поэтому
-- повторное применение ничего не меняет и не роняет.
--
-- Необязательных колонок три, и набор уже, чем у pprof_1, потому что площадка другая:
--
--   article_inputs.links     — адреса программ для перелинковки на стадии html;
--   article_inputs.seo_title — короткий заголовок выдачи (_yoast_wpseo_title);
--   article_inputs.tags      — метки записи в блоге;
--   article_metadata.tldr    — TL;DR, его рождает стадия info.
--
-- Колонок author и professions здесь нет намеренно: типа записи `author` у площадки не
-- существует (автором становится учётка публикации), а похожие профессии читал только подбор
-- связанных курсов, которого у этой темы нет. Колонок коммерческой страницы (section,
-- profession, teachers, service_name) нет тем более: поля одной задачи не имеют права
...
CREATE TABLE IF NOT EXISTS article_inputs (
    ...
    -- Поля статьи блога этой площадки.
    links TEXT,         -- адреса программ для перелинковки на стадии html
    tags TEXT,          -- метки записи в блоге

    -- Короткий заголовок для поисковой выдачи (_yoast_wpseo_title). Пусто — в выдачу идёт
    -- название статьи; имя то же, что у pprof_1 и pprof_2, и значение то же.
    seo_title TEXT
);
