-- =============================================================================
-- Схема pprof_fix_1 целиком, одним файлом.
--
-- ВАЖНО: применяется ТОЛЬКО к схеме pprof_fix_1.
--
--   docker exec -i seo-postgres psql -U seo -d seo \
--     -v ON_ERROR_STOP=1 -c 'SET search_path TO pprof_fix_1' -f - \
--     < migrations/pprof_fix_1/000001_schema.up.sql
--
-- Почему таблица одна, а не набор из article_inputs, article_outputs и article_metadata,
-- как у остальных задач: pprof_fix_1 не создаёт статью, а правит уже опубликованную. У неё
...
-- задача при первом запросе: колонок мало, и они перечислены в internal/tasks/pproffix1.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS articles (
    ...
    -- Заголовки: старый прочитан из блога, новый получен правилом переименования.
    -- Оба хранятся затем, что после записи в блог старый заголовок больше нигде не взять.
    old_title TEXT,
    new_title TEXT,
