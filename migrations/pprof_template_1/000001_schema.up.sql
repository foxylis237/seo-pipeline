-- =============================================================================
-- Схема pprof_template_1 целиком, одним файлом.
--
-- ВАЖНО: применяется ТОЛЬКО к схеме pprof_template_1 и заменяет собой общий каталог migrations/.
--
--   docker exec -i seo-postgres psql -U seo -d seo \
--     -v ON_ERROR_STOP=1 -c 'SET search_path TO pprof_template_1' -f - \
--     < migrations/pprof_template_1/000001_schema.up.sql
--
-- Файл описывает то же состояние, к которому схему pprof_template_1 привели общие миграции
