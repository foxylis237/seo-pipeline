-- =============================================================================
-- Схема pprof_fix_5 целиком, одним файлом.
--
-- ВАЖНО: применяется ТОЛЬКО к схеме pprof_fix_5.
--
--   docker exec -i seo-postgres psql -U seo -d seo \
--     -v ON_ERROR_STOP=1 -c 'SET search_path TO pprof_fix_5' -f - \
--     < migrations/pprof_fix_5/000001_schema.up.sql
--
-- Почему таблица одна, а не набор из article_inputs, article_outputs и article_metadata,
-- как у остальных задач: pprof_fix_5 не создаёт статью, а правит уже опубликованную. У неё
