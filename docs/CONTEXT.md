...
Единственный источник истины для маршрутизации — **`stage.Targets`**. Одиночная форма
`provider:` / `model:` на уровне стадии остаётся понятной YAML-парсеру, но в структуре
`LLMStageConfig` не живёт: она схлопывается в `Targets` при разборе.

---

## Схема стадий и режим статьи

- **Схема стадий** (`schemeName`) — весь набор из шести стадий с их targets и промптами.
  Схем две: `gemini` (`config/config.yaml`) и `deepseek_only` (тот же файл с наложенным
  `config/config.deepseek.yaml`).
- **Разрешённая маршрутизация** (`resolvedRouting`) — выбранная схема плюс причина выбора и
  доступность провайдеров. Ответ на вопрос «чем пойдёт эта статья», данный один раз до её
  начала. Роутер и конвейер его не пересматривают.
- **Режим статьи** — схема, выбранная для конкретной статьи. Внутри статьи не меняется.
- **Выключение провайдера** — маркер `data/llm/gemini-unavailable` со сроком и причиной.
  Живёт файлом, а не в PostgreSQL: это состояние провайдера, а не статьи, и читается до
  подключения к базе. У DeepSeek такой же маркер лежит рядом с профилем браузера.

«Схема Gemini» **не** значит «всё через Gemini»: в ней `structure`, `info` и `html` идут в
`deepseek_web`. Схема — про набор стадий целиком, target — про одну стадию. Не путать.

Выбор схемы делает только `llmResolver.Resolve`. Появление второго места, где решается тот же
вопрос, — та самая ошибка, из-за которой ломался dry-run.
