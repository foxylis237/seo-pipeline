...
func ReadArticles(path string) ([]article.Input, error) {
	...
	for rowIndex := 1; rowIndex < len(rows) && len(articles) < defaultLimit; rowIndex++ {
		...
		// ID обязателен. Пустые строки пропускаем.
		...
		input := article.Input{
			...
			Header:          cellValue(row, columnIndexes["header"]),
			ImageSlug:       cellValue(row, columnIndexes["image_slug"]),
			MetaDescription: cellValue(row, columnIndexes["meta_description"]),
			Keyword:         cellValue(row, columnIndexes["key_word"]),
			ReferenceURL:    cellValue(row, columnIndexes["reference_url"]),
			Category:        cellValue(row, columnIndexes["category"]),
			Author:          cellValue(row, columnIndexes["authors"]),
			Links:           cellValue(row, columnIndexes["links"]),
			Professions:     cellValue(row, columnIndexes["professions"]),
		}
