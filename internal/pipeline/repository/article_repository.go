...
func (r *ArticleRepository) GetResultInput(ctx context.Context, externalID string) (article.ResultInput, error) {
	...
	query := `
		SELECT a.id, a.external_id, a.title, COALESCE(i.image_slug, ''),
			...
			` + r.inputColumn("author") + `, COALESCE(i.key_word, ''),
	...
	targets := append([]any{
		...
		&input.Category, &input.Tags, &input.TLDR, &input.FAQ, &rawMetadata,
		&input.Professions, &input.Links, &input.Author,
...
// ClaimNextIncomplete atomically reserves the first article available for the task.
