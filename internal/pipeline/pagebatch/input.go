// Package pagebatch описывает пачку уже опубликованных страниц: чем она задана и когда её
// пора бросить.
//
// Здесь живут две вещи, общие у всех задач, которые работают по списку опубликованных
// адресов, — разбор входного файла и предохранитель прогона. Ни та, ни другая ничего не знает
// о том, что с найденной страницей потом сделают: правка вернёт в блог исправленный текст,
// аудит не вернёт ничего. Второй копии этих правил в проекте быть не должно — разойдутся они
// молча, а обнаружится это по недостающей статье в блоге и через недели.
package pagebatch
...
type Source struct {
	...
	Slug string
	// PostID — идентификатор записи в блоге, если он известен из входного файла.
	//
	// Ноль — законное и прежнее состояние: тогда запись ищут по слагу перебором типов, до
	// тридцати страниц по сотне записей. Известный идентификатор эту работу отменяет — это
	// один запрос вместо минуты, — но найденную по нему запись всё равно сверяют со ссылкой:
	// чужая страница внешне неотличима от своей.
	PostID int64
	// Topic — тема страницы своими словами, если она названа во входном файле.
	//
	// Пустая тема — законное состояние: основанием проверки тогда служит заголовок записи.
	// Построчный разбор её не даёт вовсе, она приходит только из книги с колонками.
	Topic string
}
