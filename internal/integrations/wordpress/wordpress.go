...
const (
	...
	//
	// Сорок пять секунд, а не десять: живой сайт с плагинами отвечает на /wp/v2/categories и
	// /wp/v2/tags заметно дольше десяти секунд, и публикация статьи 42 сорвалась дважды подряд
	// именно на этом — сначала на чтении рубрик, потом на заведении метки. Записи при этом не
	// создавалось, то есть отказ был чистым ожиданием, а не потерянным результатом.
