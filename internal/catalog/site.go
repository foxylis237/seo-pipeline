...
// Taxonomy отдаёт имя таксономии рубрик для типа записи этой площадки.
//
// Второе значение — false у типа, которого у площадки нет вовсе. Различать эти два случая
// обязательно: правило имени («cat_» + тип у одной площадки, тип + «-cat» у другой) применимо
// к любой строке и на выдуманном типе молча вернуло бы правдоподобное имя несуществующей
// таксономии — а узнал бы об этом человек уже по ненайденной рубрике в блоге.
//
// Метод нужен не сбору каталога, а публикации: у задачи, которая пишет страницы услуг этой
// площадки, тип записи приходит колонкой книги, и таксономию по нему выбирает раскладка полей.
// Второй копии списка типов и правила имени в composition root быть не должно.
