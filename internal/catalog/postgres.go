...
// PostgresStore — каталог в схеме site.
//
...
func (s *PostgresStore) Replace(ctx context.Context, programs []Program) error {
	return pgx.BeginFunc(ctx, s.pool, func(tx pgx.Tx) error {
		industries, err := saveIndustries(ctx, tx, programs)
		if err != nil {
			return err
		}
		professions, err := saveProfessions(ctx, tx, programs)
		if err != nil {
			return err
		}
		if _, err := tx.Exec(ctx, `DELETE FROM site.programs`); err != nil {
			return fmt.Errorf("очистить услуги: %w", err)
		}
		for _, program := range programs {
			_, err := tx.Exec(ctx, `
				INSERT INTO site.programs (
...
func (s *PostgresStore) List(ctx context.Context) ([]Program, error) {
	rows, err := s.pool.Query(ctx, `
		...
		FROM site.programs p
		LEFT JOIN site.industries i ON i.id = p.industry_id
		LEFT JOIN site.professions f ON f.id = p.profession_id
		LEFT JOIN site.profession_aliases a ON a.profession_id = f.id
...
func saveIndustries(ctx context.Context, tx pgx.Tx, programs []Program) (map[string]int64, error) {
	...
	for _, program := range programs {
		...
		var id int64
		err := tx.QueryRow(ctx, `
			INSERT INTO site.industries (taxonomy, term_id, slug, name, updated_at)
			VALUES ($1, $2, $3, $4, NOW())
...
func saveProfessions(ctx context.Context, tx pgx.Tx, programs []Program) (map[string]int64, error) {
	ids := make(map[string]int64)
	for _, program := range programs {
		...
		var id int64
		err := tx.QueryRow(ctx, `
			INSERT INTO site.professions (slug, name, updated_at)
			VALUES ($1, $2, NOW())
		...
		for _, alias := range profession.Aliases {
			_, err := tx.Exec(ctx, `
				INSERT INTO site.profession_aliases (profession_id, alias, source)
				VALUES ($1, $2, 'catalog')
