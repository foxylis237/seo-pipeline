...
// noticeTextJS — общая часть: текст страницы без содержимого диалога.
//
// Вырезается и то, что ответила модель, и то, что отправили мы: состояние страницы — это
// плашка площадки, а не слова внутри беседы. «Access denied» модель может написать в любой
// статье, а мы — прислать ей же в промпте.
//
// Отправленное вырезается построчно, а не целиком: в поле ввода уходит один текст, а на
// странице он отрисован своими переводами строк и склейками, и точного совпадения не будет.
// Порог длины отсекает короткие строки вроде «H2 - Обязанности»: такая строка совпала бы со
// случайным местом страницы, а длинный абзац уникален.
//
// Ловушка снята с живого прогона: статья 10 pprof_1 («Младший медперсонал») содержит фразу
// «Нарушение правил утилизации отходов класса „Б“ грозит штрафами». Эта фраза — маркер
// terms_violation, и она приезжала на страницу в нашем же промпте стадии html. Пока текст
// статьи оставался в noticeText, дописывание оборванной разметки дважды объявлялось
