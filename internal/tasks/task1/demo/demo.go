...
const (
	...
	FolderName = "DEMO"
	// FixLinksHTMLPromptPath — объединённый промпт второго сообщения ручного чата. Он
	// существует только для DEMO и боевые промпты не подменяет.
...
func NewBuilder(root string, repository Repository, artifacts Artifacts, result ResultRenderer, generator Generator, preparer Preparer, logger *slog.Logger) *Builder {
	if logger == nil {
		logger = slog.New(slog.DiscardHandler)
	}
	return &Builder{
		root: root, repository: repository, artifacts: artifacts, result: result,
		generator: generator, preparer: preparer, mergedPromptPath: FixLinksHTMLPromptPath, logger: logger,
	}
}
