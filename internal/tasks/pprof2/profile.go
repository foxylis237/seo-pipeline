...
//
// Последние пять — числа программы, и приходят они из книги намеренно. Те же значения человек
// заполняет в полях записи руками, а до появления колонок промпт выводил их из вида программы
// диапазонами: сверить диапазон с полем записи нельзя, и расхождение видно только глазами на
// самой странице. Набор и смысл у них тот же, что у obuch_2: площадки разные, поле одно.
// Незаполненная колонка — законное состояние и прежнее поведение: параметра на странице тогда
// нет вовсе, а фактуру промпт строит по правилам вида программы.
