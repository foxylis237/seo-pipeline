...
// каждое в своё место, и склеенная строка заставила бы модель разбирать её обратно. Источник
// у них один и человеческий — колонка книги: те же пять значений человек заполняет в полях
// записи руками, и выведенное моделью из вида программы число разошлось бы с админкой молча.
...
// programFact подставляет значение колонки книги или явный текст вместо него.
//
// Пустое место в промпте опаснее отсутствия строки: незаполненный параметр модель читает как
// приглашение придумать число, а прямую формулировку — как правило. Ровно по этой причине так
// же устроен раздел преподавателя ниже.
...
// missingNumber — текст на месте незаполненной колонки числа программы.
//
// У pprof_2 он мягче, чем у соседней площадки, и это прежнее поведение задачи: колонок до сих
// пор не было вовсе, и сорок с лишним живых страниц написаны по правилу вида программы —
// диапазоном часов и видом документа. Незаполненная колонка обязана оставить ровно его, иначе
// появление колонок молча изменило бы каждую следующую страницу.
func missingNumber(what string) string {
	return "в книге не указан (" + what + ") — точного значения не называй, работай по правилу вида программы"
}
...
//
// Сегодня промпт этот раздел не печатает, и числа программы тут ни при чём: объём, срок,
// стоимость, документ и аттестация пришли в книгу своими колонками и уходят в модель
// отдельными полями (articleData выше) — каждое в своё место промпта. Здесь остальное:
// место страницы в каталоге, преподаватели и уже утверждённые человеком формулировки.
// Функция остаётся готовой: она описывает, как эти факты обязаны выглядеть, когда понадобятся,
// и проверена тестами.
//
