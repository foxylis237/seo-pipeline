...
//
// Пункта 5 промпта — «дополнительная информация об услуге» — среди полей нет намеренно:
// раздел оставлен пустым до тех пор, пока данные о программе, сроках и стоимости не появятся
// в книге импорта. Сам сборщик раздела никуда не делся, это ServiceFacts ниже: вернуть его
// значит дописать поле сюда и плейсхолдер в промпт.
...
// ServiceFacts собирает раздел «дополнительная информация об услуге» основного промпта.
//
...
// остальные задачи: расхождения в проверке HTML между задачами быть не должно.
