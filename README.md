...
Реализованы две задачи. **`task_1`** — старая реализация: она остаётся рабочей и запускается через Makefile как раньше, но сохранена ради обратной совместимости и в дальнейшем будет удалена. **`pprof_1`** — актуальный пайплайн генерации поверх общего механизма задач. Новые изменения делаются в общем pipeline (`internal/pipeline`) и в задачах поверх него, а не в `task_1`. Движок у задач общий, различаются они конфигурацией и порядком стадий: см. «Задачи `task_1` и `pprof_1`».
...
- `config/pprof_2.yaml` — самостоятельная схема стадий `pprof_2`, без наложения.
- `input/task_1/`, `input/pprof_1/`, `input/pprof_2/` — каталоги Excel импорта, у каждой задачи свой. Имя книги значения не имеет: импорт берёт единственный `.xlsx` каталога.
...
- `internal/tasks/` — тип `Profile`: всё, чем одна задача отличается от другой;
  - `task1/profile.go`, `pprof1/profile.go`, `pprof2/profile.go` — конфигурация конкретной задачи;
  - `pprof1/flow.go`, `pprof1/chat.go` — поток генерации `pprof_1` и минимальный контракт чата;
  - `pprof2/flow.go`, `pprof2/chat.go` — то же для `pprof_2`; от `pprof1` пакет не зависит.
...
- `migrations/` — SQL-миграции PostgreSQL, общие всем схемам;
  - `migrations/pprof_2/` — миграции, применяемые **только** к схеме `pprof_2`.
...
- `tasks/pprof_2/prompts/` — то же для `pprof_2`; промптов `pprof_1` задача не использует.
- `tasks/<задача>/templates/` — шаблон `result.md`.
- `tasks/<задача>/output/` — создаваемые артефакты статей.
- `output/task1/`, `output/pprof_1/`, `output/pprof_2/` — отчёты импорта (`import-reports/`) и диагностика неудачных попыток Keys.so, Arsenkin, DeepSeek и Google (`debug/`). У каждой задачи свои: `reset` одной не трогает данные другой.

## Задачи `task_1`, `pprof_1` и `pprof_2`

Задача — это отдельный пайплайн со своими каталогами, промптами, схемой стадий и схемой PostgreSQL. Набор команд у задач общий: `make <task> <операция>`, где `<task>` — `task-1`, `pprof-1` или `pprof-2`.

> **`task_1` — устаревшая реализация.** Все её команды продолжают работать, но она сохранена только ради обратной совместимости: из `make help` её планируется скрыть, а после переезда оставшихся сценариев на общий механизм задач — удалить вместе с каталогами `tasks/task_1/` и `internal/tasks/task1`. Всё новое добавляется в общий pipeline и в задачи поверх него; правки в `task_1` — только чинящие уже работающее.

Общий у них движок (`internal/pipeline`): импорт, репозиторий, атомарная публикация артефактов, диагностика, сборка `result.md`, интеграции Keys.so, Arsenkin и Google. Различается конфигурация — она живёт в `internal/tasks/task1`, `internal/tasks/pprof1` и `internal/tasks/pprof2`.

Задачи друг от друга не зависят: `pprof_2` не импортирует ни одного пакета `pprof_1` и наоборот. Общее у них — движок, а не соседняя задача.

| Что | `task_1` | `pprof_1` | `pprof_2` |
|---|---|---|---|
| Что пишет | статьи блога | статьи блога | **коммерческие страницы услуг** |
| Провайдеры | Gemini + DeepSeek Web, схема выбирается на статью | только DeepSeek Web | только DeepSeek Web |
| Схемы стадий | `config/config.yaml` + наложение `config.deepseek.yaml` | `config/pprof_1.yaml`, одна | `config/pprof_2.yaml`, одна |
| Стадии | `structure`, `article`, `info`, `review`, `fix`, `html` | `structure`, `expert`, `seo_editor`, `info`, `review`, `html` | `structure`, `article`, `html` |
| Стадия `info` | есть | есть | **нет**: FAQ разбирается из текста страницы, TL;DR нет |
| Перелинковка | стадия `html` | стадия `html` | **нет**: ссылки в разметке запрещены |
| Промпты | `tasks/task_1/prompts/` | `tasks/pprof_1/prompts/` | `tasks/pprof_2/prompts/` |
| Артефакты | `tasks/task_1/output/` | `tasks/pprof_1/output/` | `tasks/pprof_2/output/` |
| Схема PostgreSQL | `public` | `pprof_1` | `pprof_2` |
| Колонки `article_inputs` сверх обязательных | `author`, `links`, `professions`, `tags` | `author`, `links`, `professions`, `tags` | `seo_title`, `section`, `profession`, `teachers`, `service_name` |
| Схема PostgreSQL собирается из | `migrations/task_1/` | `migrations/pprof_1/` | `migrations/pprof_2/` |
| Excel | `input/task_1/*.xlsx` | `input/pprof_1/*.xlsx` | `input/pprof_2/*.xlsx` |
| Префикс переменных | нет | `PPROF_1_` | `PPROF_2_` |
| Папка Google Drive | общая | общая | своя |

## Задача `pprof_fix_1` — правка уже опубликованных статей

Четвёртая задача стоит особняком: она не пишет статью, а **изменяет существующую запись блога** — заголовок и текст. Отсюда и всё остальное: вход не Excel, а файл со ссылками; таблица одна вместо пяти; стадия одна; ни research, ни структуры, ни метаданных, ни обложек у неё нет.

```text
make pprof-fix-1 import        индексы и ссылки из файла в input/pprof_fix_1/
make pprof-fix-1 run plan [ID] что изменится в блоге — читает статьи, ничего не пишет
make pprof-fix-1 run [ID]      правка и запись в блог; без ID — все непереписанные
```

Прогон одной статьи:

```text
fetch    найти запись по слагу из ссылки, прочитать заголовок и тело
         → original/article.html, original/title.txt   (оригинал на диск ДО любой записи)
rewrite  промпт правки + статья → DeepSeek → исправленный HTML целиком
         → prompts/rewrite_prompt.txt, generated/article.html
update   wp.editPost: новый заголовок и новый текст в ту же запись, затем обратная сверка
result   → result.md: название статьи и ссылка на неё
```

Заголовок модель не пишет. Он меняется правилом из `tasks/pprof_fix_1/title_rule.txt`, где задана одна пара «было — стало»: код сам выводит, что именно изменилось, и применяет ту же правку к каждой статье. Заголовок без якоря останавливает статью до запроса к модели, уже переименованный остаётся как есть.

Заголовков у страницы три, и меняются они вместе:

| Где | Что это | Меняется |
|---|---|---|
| `post_title` | название записи, видно в админке | всегда |
| `prof_title` | поле темы, из него рисуется **H1 на самой странице** | всегда, если поле есть; правило не подошло — статья останавливается до запроса к модели |
| `_yoast_wpseo_title` | заголовок для поисковой выдачи | если правило к нему подходит; формулировка там своя и короче |

Без `prof_title` правка выглядела бы сделанной, а на странице остался бы прежний H1 — поэтому поле сверяется после записи наравне с текстом. Поля отправляются с идентификатором postmeta: без него WordPress не обновил бы значение, а завёл второе поле с тем же ключом.

| Что | `pprof_fix_1` |
|---|---|
| Что делает | меняет заголовок и текст **существующей** записи |
| Вход | `input/pprof_fix_1/` — один файл со строками «индекс + ссылка»: HTML-таблица, CSV или список строк |
| Стадии | одна: `rewrite` |
| Схема стадий | `config/pprof_fix_1.yaml` |
| Промпт | `tasks/pprof_fix_1/prompts/rewrite.txt` |
| Артефакты | `tasks/pprof_fix_1/output/<индекс>-<слаг>/` |
| Схема PostgreSQL | `pprof_fix_1`, одна таблица `articles` (`migrations/pprof_fix_1/`) |
| Префикс переменных | `PPROF_FIX_1_` |
| Операции | только `import` и `run`; `publish`, `prepare`, `regenerate` и прочие ей не принадлежат и отбиваются на старте |

Две защиты от необратимого. Первая — оригинал: старый текст и старый заголовок ложатся на диск раньше, чем в блог уходит правка, потому что записи в блоге приложение не восстанавливает. Вторая — отметка `updated_post_at`: переписанную статью повторный `run` не трогает, иначе модель получила бы на вход собственный вывод.

Обрыв ответа лечится так же, как у остальных задач: продолжением того же чата, до двух раз. Признак обрыва свой — правка вправе переписать последний абзац, поэтому сверяется структура: последний заголовок исходной статьи обязан найтись в ответе, а заголовков должно быть не меньше, чем было.
...
`pprof_2` пишет не статьи блога, а **коммерческие страницы услуг** учебного центра: что это за услуга, кому подходит, требования, формат, сроки, стоимость, документ, где применяется квалификация. Отсюда все её отличия от `pprof_1` — другой основной промпт, другой набор полей Excel и другой `result.md`.

Профиль независим полностью: свои каталоги, своя схема PostgreSQL, свои промпты, свой префикс переменных и своя папка Google Drive. Ни один файл `pprof_1` она не читает и ни одной его таблицы не трогает.

### Поток генерации: три чата

```text
Чат 1   structure          → generated/structure.txt
Чат 2   article            → article.txt
          ↓
        разбор текста      → FAQ в article_metadata
Чат 3   html               → article.html
                           → result.md
```

Границы между чатами значимы: первое сообщение каждого просит провайдера начать новую беседу, поэтому истории не смешиваются.

Чат 2 сегодня состоит из одного сообщения — страницу целиком пишет основной промпт. SEO-редактура и ревью вернутся следующими сообщениями того же чата, поэтому он и остаётся отдельным: их промпты пока лежат заглушками, а стадий в `config/pprof_2.yaml` нет. Команды `article`, `info`, `review` и `fix` ведут в один и тот же чат 2.

Файл у страницы один — `generated/article.txt`. Слоты `review_path` и `fixed_article_path` указывают на него же: ревью не было, и финальный текст — это текст основного промпта. Пустыми их оставлять нельзя, иначе раннер полного прогона возвращался бы на этап `article` вечно.

**FAQ берётся из самой страницы.** Стадии `info` у `pprof_2` нет и не будет: частые вопросы уже написаны в тексте — их требует структура, — и второй запрос к модели дал бы второй, отличный от опубликованного, набор. После чата 2 блок вопросов вынимается разбором (`pprof2.ExtractFAQ`) и сохраняется в `article_metadata.faq` — до стадии `html`, поэтому разметка вправе убрать блок со страницы: в блоге его собирает тема из полей `blog_faq_*`. TL;DR и время чтения задача не генерирует вовсе.

Пустой FAQ генерацию не роняет — страница написана и оплачена, — но и не прячется: в лог уходит предупреждение, а публиковать статью без вопросов не даст проверка публикации.

### Промпты

```text
tasks/pprof_2/prompts/
  keywords.txt       резервный подбор запросов для prepare
  structure.txt      чат 1: структура коммерческой страницы
  article.txt        чат 2: основной промпт, он же автор страницы
  html.txt           чат 3: разметка по регламенту, без перелинковки
  seo_editor.txt     заглушка: стадия вне потока
  review.txt         заглушка: ревью вернётся после article
```

**Основной промпт уходит в модель.** Здесь `pprof_2` отличается от `pprof_1`: там `article.txt` — только артефакт и документ Google, здесь тот же файл пишет страницу. Тот же отрендеренный текст сохраняется в `prompts/article_prompt.txt` и выгружается в Google Docs. Копия одна, разойтись ей негде.

Пункт 5 промпта — «дополнительная информация об услуге» — сейчас пуст: фактов о программе, сроках и стоимости в книге импорта нет, а придумывать их промпт запрещает. Сборщик раздела остаётся готовым (`pprof2.ServiceFacts`), вернуть его — дописать поле в `articleData` и плейсхолдер в промпт.

### Колонки Excel

| Excel | Поле | Куда идёт |
|---|---|---|
| `id` | `external_id` | идентификатор во всех командах |
| `article_name` | заголовок статьи | раздел «Название», заголовок записи блога |
| `service_name` | `service_name` | раздел «Название услуги» — сама услуга, без уточнений |
| `slug` | `image_slug` | каталог артефактов и «URL картинки» |
| `header` | `header` | «Заголовок (H1)» и «Alt главной картинки» |
| `seo_title` | `seo_title` | «SEO-заголовок» |
| `meta_description` | `meta_description` | «Мета-описание» |
| `key_word` | `key_word` | «Фокусное ключевое слово» |
| `reference_url` | `reference_url` | вход Keys.so на этапе `prepare` |
| `category` | `category` | «Категория» |
| `section` | `section` | «Рубрика» |
| `profession` | `profession` | «Название профессии» |
| `teachers` | `teachers` | «Преподаватели»; ими же заполняется `author` |

Обязательны `id`, `article_name`, `slug`, `reference_url`. `article_name` и `service_name` — разные значения: полное название страницы и короткое название услуги; заголовком статьи становится первое. Часть заголовков книга называет иначе, чем движок (`slug` вместо `image_slug`, `teachers` вместо `authors`), это разбирает таблица синонимов импортёра. Подробности — в `input/pprof_2/README.md`.

### Свои колонки в PostgreSQL

`seo_title`, `section`, `profession`, `teachers` и `service_name` есть **только** в схеме `pprof_2`; `author`, `links`, `professions`, `tags` и `article_metadata.tldr` — наоборот, есть у `task_1` и `pprof_1`, но не у неё. Схему задачи целиком описывает `migrations/pprof_2/000001_schema.up.sql`, и применяется он **вместо** общего каталога, а не вдобавок: `repository.ValidateSchema` сверяет схему в обе стороны, и лишняя колонка для задачи такая же ошибка, как отсутствующая. Раскладка миграций — `migrations/README.md`.

Список необщих колонок задача объявляет в своём профиле (`Profile.ExtraInputColumns`), оттуда он уходит и в проверку схемы, и в репозиторий: запись и чтение называют ровно те колонки, которые в этой схеме есть. Опечатка в списке роняет команду на старте, а не всплывает пустым полем в `result.md`.

Базовый набор колонок `article_inputs` при этом общий у всех задач — это контракт движка, и расширять его ради одной задачи не нужно.

### `result.md`

Разделы: Название, Название услуги, Рубрика, Категория, Фокусное ключевое слово, Преподаватели, SEO-заголовок, Alt главной картинки, Мета-описание, Заголовок (H1), Название профессии, Вопрос N (по числу разобранных из страницы), URL картинки, Файл статьи, HTML, Гугл Док, Ссылка на статью.

Времени чтения здесь нет намеренно — его не считают и не хранят. TL;DR, меток и похожих профессий тоже нет: TL;DR задача не генерирует, а меток и списка профессий нет ни в книге, ни в её схеме. FAQ есть, но приходит не от стадии `info`, а разбором написанной страницы.

Шаблон — `tasks/pprof_2/templates/result.md.tmpl`. «Рубрика» и «Категория» живут порознь: это два разных уровня каталога.

### Первый запуск `pprof_2`

```bash
# 1. Схема PostgreSQL и её таблицы — только своя baseline, общие миграции не накатывать
...
# 2. Входные данные
cp <ваша книга>.xlsx input/pprof_2/
cp <регламент вёрстки>.pdf input/pprof_2/regulations/html/
cp <обложки>.webp input/pprof_2/images/     # имя файла — external_id

# 3. Импорт и сверка — ни денег, ни внешних сервисов
make pprof-2 import
make pprof-2 import-check
```

Дальше `make pprof-2 prepare` и `make pprof-2 run` — они **тратят деньги** и ходят в Keys.so, Arsenkin и DeepSeek.

### Переменные окружения `pprof_2`

Все с префиксом `PPROF_2_`; без него переменная принадлежит `task_1` и в `pprof_2` не протекает.

| Переменная | Умолчание | Зачем |
|---|---|---|
| `PPROF_2_OUTPUT_DIR` | `tasks/pprof_2/output` | корень артефактов |
| `PPROF_2_INPUT_FILE_PATH` | не задана | явная книга импорта вместо поиска в `input/pprof_2/` |
| `PPROF_2_DATABASE_URL` | `DATABASE_URL` | другой сервер PostgreSQL для этой задачи |
| `PPROF_2_WORDPRESS_URL` | не задана | площадка публикации (публикация ещё не настроена) |
| `PPROF_2_WORDPRESS_USERNAME` | не задана | логин площадки |
| `PPROF_2_WORDPRESS_APP_PASSWORD` | не задана | Application Password площадки |
| `PPROF_2_WORDPRESS_PUBLISH_AFTER_RUN` | `false` | публиковать ли сразу после `run` |

Keys.so, Arsenkin и Google берутся из общих переменных: аккаунты у задач одни и те же.
...
# WordPress                         →
make pprof-1 wordpress-check        # проверка доступа к площадке, без записи
make pprof-1 publish plan 45        # что уйдёт в WordPress, без записи
make pprof-1 publish plan           # проверить все готовые статьи разом
make pprof-1 publish 45             # опубликовать одну статью
make pprof-1 publish                # все готовые и ещё не опубликованные, с подтверждением
make pprof-1 mark-published 45 21593  # привязать к уже существующей записи блога

# DEMO для ручного прогона          $
make pprof-1 demo-generate          # пересобрать DEMO всем статьям
make pprof-1 demo-generate 45       # одной статье
...
Публикация — последний этап `run`, но только если её включили: `PPROF_1_WORDPRESS_PUBLISH_AFTER_RUN=true` в `.env`. По умолчанию `run` ничего не публикует. Когда включена — статья публикуется сразу после успешного `html` и сборки `result.md`; отказ площадки генерацию не роняет, статья с негодными данными просто пропускается, а после трёх подряд отказов площадки публикация в этом прогоне выключается — остальные статьи всё равно проходят все этапы, и выложить их можно командой `publish`.

Рубрика и метки берутся из колонок `category` и `tags`. Рубрика обязана существовать в блоге: их два десятка, они продуманы человеком, и ненайденная рубрика останавливает публикацию. Метки, наоборот, заводятся сами: если метки из `tags` в блоге ещё нет, `publish` создаёт её перед созданием записи и пишет об этом в лог (`метка заведена в WordPress`). Дублей это не создаёт — метка сначала ищется, а на гонку WordPress отвечает `term_exists` с идентификатором уже существующей. Опечатка в `tags` при этом превращается в мусорную метку, удалять которую придётся руками: какие метки будут заведены, показывает `publish plan`.

Обложка записи — `input/pprof_1/images/<external_id>.webp` (см. README рядом с картинками). Без неё публикация отказывается работать до первого запроса в WordPress; файл уходит в медиабиблиотеку под slug'ом статьи и назначается изображением записи тем же вызовом, что создаёт её.

#### `pprof_2`

Перед первым запуском — один раз создать схему PostgreSQL и накатить в неё общие миграции плюс `migrations/pprof_2/` (см. «Первый запуск `pprof_2`»).

```bash
# Импорт и сверка
make pprof-2 import                 # весь Excel из input/pprof_2/
make pprof-2 import 10              # только первые 10 новых строк
make pprof-2 import-check           # сверить импорт с Excel
make pprof-2 import-check 5         # сверить одну страницу

# Ключевые запросы вручную
make pprof-2 keywords 5             # спросит запросы и заменит ими сбор Keys.so

# Сбор research                     →
make pprof-2 prepare                # все страницы без research
make pprof-2 prepare 5              # одна страница

# Генерация                         $
make pprof-2 generate               # полный прогон: у pprof_2 это то же, что run
make pprof-2 generate 5
make pprof-2 article                # чат 2: основной промпт и разбор FAQ из страницы
make pprof-2 article 5
make pprof-2 review                 # то же самое: своей стадии ревью пока нет
make pprof-2 fix                    # то же самое, чат 2 целиком
make pprof-2 info                   # то же самое; стадии info у pprof_2 нет
make pprof-2 html                   # чат 3: разметка по регламенту, без ссылок
make pprof-2 html 5

# Полный прогон                     $ →
make pprof-2 run                    # все незавершённые страницы, с возобновлением
make pprof-2 run 5                  # одна страница
make pprof-2 run plan               # где возобновится, ничего не запуская
make pprof-2 run plan 5
make pprof-2 retry                  # снять ошибку и прогнать
make pprof-2 regenerate 5           # пересоздать страницу целиком, research сохраняется

# Сборка результата
make pprof-2 result                 # собрать result.md всем, кому нужно
make pprof-2 result 5               # одной странице

# Диагностика
make pprof-2 errors                 # страницы с текущей сохранённой ошибкой
make pprof-2 errors 5               # по одной странице
make pprof-2 dry-run                # офлайн-прогон; требует схемы pprof_2 на 5433

# Google Docs                       →
make pprof-2 google-publish         # промпты всех страниц, у которых он сохранён
make pprof-2 google-publish 5       # одной страницы

# Очистка                           !
make pprof-2 clear 5                # вернуть одну страницу к состоянию после импорта
make pprof-2 reset 5                # вернуть одну страницу к состоянию до импорта
make pprof-2 reset                  # стереть состояние только pprof_2
```

Команды `publish`, `wordpress-check` и `mark-published` у `pprof_2` тоже приняты — они общие у всех задач, — но площадка ей ещё не настроена: пока не заданы `PPROF_2_WORDPRESS_*`, `wordpress-check` честно скажет, какой переменной не хватает. Публикация `pprof_2` — следующий шаг, а не готовая возможность.

`demo-generate` у `pprof_2` работает: DEMO у задач общий, и промпты стадий ему даёт роутер самой задачи.

Стадии `info` у `pprof_2` нет, поэтому `make pprof-2 info` — просто ещё одно имя чата 2, как `article`, `review` и `fix`. FAQ появится при любом из них: он разбирается из написанной страницы. TL;DR не появится ни при одном.
...
| `task-1 keywords` | Спрашивает ключевые запросы и кладёт их в статью вместо сбора запросов у конкурента: первый этап Keys.so считается выполненным. Ввод — по одному запросу в строке, конец — пустая строка или Ctrl-D (из файла или конвейера — конец файла, пустые строки чтение не обрывают); из строки берётся первый столбец до табуляции, поэтому колонку можно вставлять прямо из Excel вместе с частотностями. Повторы (без учёта регистра) отбрасываются со счётчиком, а о символах, которых форма Wordstat не принимает, команда предупреждает — при отправке они заменятся пробелами. Перезапись безусловная: прежние запросы и research стираются, статья возвращается к статусу `pending` и этапу `arsenkin_collection`, ошибка снимается. Готовые файлы генерации остаются — текст под новые запросы пересобирает `regenerate`. Ничего не запускает и наружу не ходит; чистку от дублей, Arsenkin и остальное делает следующий `run`. | **Обязательный** `external_id`. | `make task-1 keywords 37` | `go run ./cmd/seo-pipeline task-1 keywords <external_id>` |
...
`postgres` доступен на `localhost:5432`, `postgres-dry-run` — на `localhost:5433`. Оба контейнера применяют `migrations/task_1/*.up.sql` в `public` при первой инициализации своих volumes; схемы `pprof_1` и `pprof_2` создаются руками своими файлами. Отдельного migration runner и Makefile-цели миграции в проекте нет; уже существующий volume автоматически повторно не мигрируется.
...
Резервный подбор отдаёт не более 49 запросов и **отбрасывает** строки, в которых есть что-то кроме букв, цифр и пробелов: в ответе модели такая строка почти всегда нумерация, маркер списка или пояснение, а не запрос.

Сами фразы чистятся один раз и в одном месте — в клиенте Arsenkin, перед формой Wordstat (`sanitizeWordstatQuery`, `internal/integrations/arsenkin/client.go`). Всё, кроме букв и цифр, заменяется пробелом, повторные пробелы схлопываются, а дубли, которые от этого появились, снимаются общим отбором повторов. Точка одна намеренно: источников списка три, а требование ставит сама форма — операторные символы (`-`, `/`, `"`, `!`) она принимает молча, а задачу по ним не создаёт вовсе. Очистка Keys.so эту ловушку не закрывает: `delete-double` убирает дубли, символы не трогает.

Символ заменяется пробелом, а не выбрасывается: «seo-продвижение» обязано остаться двумя словами. В `cleaned_keywords` запросы остаются такими, какими пришли от источника, — чистка нужна форме, а не тексту статьи; что именно вычищено, видно в логе прогона по стадии `wordstat_sanitize` (счётчик и до пяти примеров).
...
Arsenkin выполняет Wordstat и Copywriters в одном browser context. Перед отправкой список нормализуется: из фраз вычищаются символы, которых форма не принимает (см. «Источник исходных запросов»), пустые строки и повторы отбрасываются, и только потом список обрезается до 49 запросов — лишние отбрасываются с записью в лог. Отправляемый набор диагностика спрашивает у самого клиента (`arsenkin.SubmittedQueries`), поэтому отчёт и форма разойтись не могут. Возвращённые фразы проверяются на принадлежность отправленному списку — результат проверки попадает в `prepare/prepare-report.json` как `wordstat_membership`.
...
В корне `DEMO` лежит то, что открывают первым: готовый `result.md` и `fix_links_html_prompt.txt` — объединённый промпт второго сообщения ручного чата. Он собирается из `tasks/common/prompts/demo/fix_links_html.txt`: этот файл общий у всех задач, существует только для DEMO и боевые промпты не подменяет.

Промпт статьи лежит в `DEMO/prompts/article_prompt.txt` — там же, где в боевом каталоге, и отличается от него ровно каталогом `DEMO`. Раскладка одна намеренно: публикация в Google Docs забирает промпт по одному правилу и после боевого прогона, и после `demo-generate`, а второе имя того же артефакта означало бы, что находится он не всегда.

`demo-generate` доступен обеим задачам. Сборщик общий (`internal/pipeline/demo`) и о задачах не знает: своими у задачи остаются только промпты стадий, и подставляет их её собственный роутер.
...
Для остальных задач — `make pprof-1 dry-run` и `make pprof-2 dry-run`. Каждая идёт в свою схему базы на `localhost:5433`, поэтому её нужно там создать заранее так же, как в основной базе. Книгу импорта прогон берёт ту же, что и боевой запуск: единственный `.xlsx` каталога задачи либо `<ПРЕФИКС>INPUT_FILE_PATH`.
...
**Показывает разрешённую маршрутизацию** — какой режим будет выбран, доступен ли каждый провайдер и почему, какой провайдер и какой таймаут у каждой стадии. Стадии перечисляются те, что объявлены профилем задачи: у `pprof_2` в отчёте есть `seo_editor` и нет `info` с `fix`, которых у него не существует. Схему выбирает тот же резолвер, что и боевой прогон, поэтому отчёт отвечает не на вопрос «как настроено», а на вопрос «как пойдёт». Доступность определяется по локальным признакам — переменным окружения и маркерам выключения; ни сетевых запросов, ни запуска браузера здесь нет.
...
**Прогоняет эту же схему целиком** на детерминированных локальных ответах: импорт, все стадии задачи, сборка `result.md` и проверка, что каждый артефакт на месте. Поток берётся тот же, что и в бою: `task_1` идёт общим конвейером, `pprof_1` и `pprof_2` — своими тремя чатами. Ответ заглушки выбирается по стадии, а не по модели: в чате все сообщения после первого идут к одному провайдеру, и по модели стадию уже не отличить.

Регламент вёрстки стадии `html` офлайн-прогону не обязателен: заглушка документы принимает, а ненайденный файл прогон не роняет — он назван отдельной строкой отчёта маршрутизации.
...
Оба списка закреплены `CHECK`-ограничениями в baseline каждой задачи (`migrations/<задача>/000001_schema.up.sql`). Новый этап требует новой миграции и правки `expectedSchema` в `internal/pipeline/repository/schema.go`.
...
Изоляция задач в PostgreSQL держится на `search_path`: `task_1` работает в `public`, `pprof_1` — в схеме `pprof_1`, `pprof_2` — в `pprof_2`. Профиль задачи дописывает `search_path` в `DATABASE_URL`, поэтому отдельный DSN не нужен, а `public` возвращается нетронутым — подключение `task_1` не изменилось. Имя схемы проверяется до подстановки в строку подключения и берётся только из профиля.

Набор таблиц у задач один и тот же, а набор колонок — нет. Обязателен только общий минимум `article_inputs`: `category`, `header`, `image_slug`, `meta_description`, `key_word`, `reference_url`. Всё остальное задача объявляет в профиле (`Profile.ExtraInputColumns`) — и то, что есть только у неё, и то, чего у неё нет, хотя есть у соседей.

Схему каждой задачи описывает один файл в её каталоге, и применяется он **вместо** чужих, а не вдобавок:

```text
migrations/task_1/000001_schema.up.sql    схема public
migrations/pprof_1/000001_schema.up.sql   схема pprof_1
migrations/pprof_2/000001_schema.up.sql   схема pprof_2
```

Список из профиля уходит одновременно в `repository.ValidateSchema` и в репозиторий, поэтому проверка схемы, `INSERT` и `SELECT` называют ровно те колонки, которые у задачи есть. Проверка строгая в обе стороны: у задачи, объявившей колонку, её отсутствие — «missing column», а у задачи, не объявлявшей, лишняя колонка — «unexpected column». Отсюда правило: **каталоги миграций не смешиваются** — файл `task_1`, применённый к схеме `pprof_2`, заведёт в ней колонки статей блога, и та перестанет стартовать на «unexpected column». Подробности раскладки — `migrations/README.md`.

| Схема | Колонки `article_inputs` сверх обязательных | `article_metadata.tldr` |
|---|---|---|
| `public` (`task_1`) | `author`, `links`, `professions`, `tags` | есть |
| `pprof_1` | `author`, `links`, `professions`, `tags` | есть |
| `pprof_2` | `seo_title`, `section`, `profession`, `teachers`, `service_name` | нет |

## Текущие ограничения

- `task_1` — устаревшая реализация, сохранённая для обратной совместимости. Работает как раньше, но развитие идёт в общем pipeline; в перспективе задача будет удалена.
- `pprof_1` и `pprof_2` работают только через DeepSeek Web: Gemini и другие провайдеры им не настроены.
- У `pprof_2` нет стадии `info`: TL;DR не генерируется вовсе, а FAQ приходит разбором написанной страницы, а не запросом к модели.
- Публикация `pprof_2` в блог сейчас невозможна: `publish` требует непустые метки, а колонки `tags` у задачи нет.
- Времени чтения в `result.md` у `pprof_2` нет и не считается.
- Публикация `pprof_2` в WordPress подключена, но не настроена: команды проходят весь локальный путь, а переменные `PPROF_2_WORDPRESS_*` пока пусты. TL;DR, FAQ и время чтения у задачи без стадии `info` не требуются и в запись не уходят — поля `blog_tldr`, `blog_read`, `blog_faq` не отправляются. Рубрика, метки и обложка обязательны по-прежнему: без колонки `tags` в книге `publish` откажет до первого запроса в блог. Раскладка полей коммерческой страницы задаётся в `wordPressCustomFields`.
- В книге `pprof_2` нет колонок `tags` и `professions`: меток для блога и списка похожих профессий у неё пока не появится.
- В `pprof_1` и `pprof_2` `article`, `info`, `review` и `fix` — четыре имени одного действия: чат 2 неделим и прогоняется целиком, поэтому отдельно переделать только ревью нельзя.
...
- Перед отправкой в Wordstat из фраз вычищается всё, кроме букв, цифр и пробелов: символ заменяется пробелом, а не удаляется, и появившиеся дубли снимаются. В `cleaned_keywords` запросы остаются как есть.
