...
help: ## этот список
	...
	@printf '  %-34s%s\n' \
		...
		'' '' \
		'obuch-1 — те же статьи блога, другая площадка:' '' \
		'make obuch-1 import'              'импорт статей из Excel [limit]' \
		'make obuch-1 prepare'             'research Keys.so и Arsenkin [ID]' \
		'make obuch-1 run'                 'полный прогон, возобновляемый [ID]' \
		'make obuch-1 article'             'чат 2 целиком [ID]' \
		'make obuch-1 html'                'чат 3: HTML и перелинковка [ID]' \
